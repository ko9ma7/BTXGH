...
def get_img_src(name):
    url = "https://instagram.com/" + str(name)
    driver.get(url)
    soup = bs(driver.page_source, 'lxml')
...
def get_img_src_prv(name):
    ...
    # Profile Pic
    divImage = soup.find('img', {"class": "be6sR"})
...
df = pd.read_csv('../influencer_commenters.csv', encoding='ascii')
df['commenters_s'] = df['commenters'].apply(list_format)
print("changed df:", df.head(3))
...
for index, row in df.iterrows():
    ...
    print(t0)
    influencer = row['name']
    print(index, influencer)
    commenters = row['commenters_s'] # Which is an array of names
    ...
    for i in range(len(commenters)):
        try:
            try:
                ...
                img_src = get_img_src_prv(commenters[i])
                each.append(img_src)
                print(i, 'out of', len(commenters))
        except:
            continue
